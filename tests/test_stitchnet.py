...
def test_generate():
    ...
    score,net = generate(stitching_dataset, threshold=0.8, totalThreshold=0, maxDepth=10, K=1, sample=True)    
    assert abs(score - 0.99) < 1e-5
...
def test_evaluate_dataset():
    ...
    assert abs(result['accuracy'] - 0.796875) < 1e-5